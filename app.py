...
try:
    from utilities.postgres_schema import ensure_roadmap_comments_table
    ensure_roadmap_comments_table()
except Exception as e:
    logger.warning(f"⚠️ crab.roadmap_comments init deferred: {e}")
...
try:
    from utilities.area_schema import init_area_schema
    init_area_schema()
except Exception as e:
    logger.warning(f"⚠️ crab.area_* schema init deferred: {e}")

try:
    from flight_price_history import ensure_flight_price_daily
    _fpd_conn = get_db_connection()
    try:
        ensure_flight_price_daily(_fpd_conn)
    finally:
        _fpd_conn.close()
except Exception as e:
    logger.warning(f"⚠️ kumori_ops.flight_price_daily init deferred: {e}")
...
def api_roadmap_comments_get():
    ...
    try:
        ...
        # Table is ensured once at startup (ensure_roadmap_comments_table).
        cur.execute("""
...
def trip_summary(plan_id):
    ...
    from plan_routes import _auto_login_demo_viewer
...
from flight_hunter_routes import bp as flight_hunter_bp
...
from area_routes import bp as area_bp
from flight_price_history import bp as flight_price_history_bp
from travel_agent_routes import bp as travel_agent_bp
...
app.register_blueprint(flight_hunter_bp)
app.register_blueprint(shorturl_bp)
app.register_blueprint(timeshare_bp)
app.register_blueprint(travel_agent_bp)
app.register_blueprint(heartbeat_bp)
app.register_blueprint(area_bp)
app.register_blueprint(flight_price_history_bp)
...
# ── Eagerly init kumori_api_client at startup (post 2026-05-10 HTTP migration) ──

try:
    from utilities import kumori_api_client
    kumori_api_client.init(
        get_secret_fn=get_secret,
        api_key_name='CRAB_KUMORI_API_KEY',
    )
    logger.info('kumori_api_client initialized on startup')
except Exception as e:
    logger.warning(f'kumori_api_client init deferred: {e}')
