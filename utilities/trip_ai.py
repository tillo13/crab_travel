import json
import logging
from utilities.kumori_api_client import llm_generate as free_generate
...
Be specific — use real places if you know the destination, otherwise create realistic suggestions."""
    ...
    try:
        text, backend = free_generate(full_prompt, max_tokens=4096, temperature=0.7)
        if not text:
...
compatibility_score: 1-100 based on how well this destination fits the group.{vibes_instruction}"""
    ...
    try:
        text, backend = free_generate(full_prompt, max_tokens=3000, temperature=0.7)
        if not text:
            return None
        text = text.strip()
        if text.startswith('```'):
...
Be creative but practical. Mix well-known and unexpected destinations. Consider the group's interests and budget."""
    ...
    try:
        text, backend = free_generate(full_prompt, max_tokens=1024, temperature=0.8)
        if not text:
            return []
        text = text.strip()
        if text.startswith('```'):
