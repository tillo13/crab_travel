...
@ensure_once
def ensure_roadmap_comments_table():
    """Roadmap-page comment store. Extracted from the /api/roadmap/comments
    GET route (where the DDL ran on every page load) into a once-per-process
    startup ensure — db-speed-first."""
    ...
    try:
        ...
        """)
        conn.commit()
        logger.info("crab.roadmap_comments table ready")
    finally:
        conn.close()
...
def init_database():
    ...
    try:
        ...
        # Notification dedupe — vote reminders + suppress-on-unchanged status emails
        # (daily_heartbeat, openclaw_admin_digest) store a state_hash here so an
        # identical email isn't re-sent every day. channel is varchar(32): the old
        # varchar(10) truncated 'email_suppressed' and 500'd /api/opencrab/notify.
        cursor.execute("""
        ...
        # Idempotent migrations for tables created by an older deploy.
        cursor.execute("ALTER TABLE crab.notifications_sent ADD COLUMN IF NOT EXISTS state_hash VARCHAR(64)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_notif_sent_type_sent_at ON crab.notifications_sent (notification_type, sent_at DESC)")
        cursor.execute("""
            SELECT character_maximum_length FROM information_schema.columns
            WHERE table_schema='crab' AND table_name='notifications_sent' AND column_name='channel'
        """)
        _ch = cursor.fetchone()
        if _ch and _ch[0] is not None and _ch[0] < 32:
            cursor.execute("ALTER TABLE crab.notifications_sent ALTER COLUMN channel TYPE VARCHAR(32)")

        # Price-drop alert ledger. The gate (_send_alert_v2) is a true price FLOOR:
        # an alert fires only if its price is materially below the lowest price
        # already alerted for this watch+channel within the retention window. The
        # old (price_band, dedupe_bucket) UNIQUE key re-fired the same deal on a
        # Thursday epoch rollover or a $25 band straddle — retired below.
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS crab.watch_alerts (
                pk_id BIGSERIAL PRIMARY KEY,
                watch_id BIGINT NOT NULL REFERENCES crab.member_watches(pk_id) ON DELETE CASCADE,
                baseline_usd NUMERIC(10,2) NOT NULL,
                alert_usd NUMERIC(10,2) NOT NULL,
                drop_pct NUMERIC(5,2) NOT NULL,
                sources_corroborating INTEGER NOT NULL,
                price_band INTEGER NOT NULL,
                channel VARCHAR(10) NOT NULL,
                deep_link TEXT,
                verified BOOLEAN DEFAULT FALSE,
                verified_at_price NUMERIC(10,2),
                sent_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                dedupe_bucket BIGINT  -- deprecated; kept nullable for back-compat
            )
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_watch_alerts_watch ON crab.watch_alerts(watch_id, sent_at DESC)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_watch_alerts_watch_chan ON crab.watch_alerts(watch_id, channel, sent_at DESC)")
        # Retire the fixed-week+band unique key (it BLOCKED legitimate new-low
        # re-alerts) and free dedupe_bucket from NOT NULL so the new floor gate's
        # insert (which doesn't set it) succeeds. Idempotent — safe every startup.
        cursor.execute("DROP INDEX IF EXISTS crab.idx_watch_alerts_dedupe")
        cursor.execute("ALTER TABLE crab.watch_alerts ALTER COLUMN dedupe_bucket DROP NOT NULL")

        # Why-an-alert-didn't-fire ledger. Surfaced in OpenClaw daily digest so
        # adapter quote-quality issues are visible without polluting user inbox.
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS crab.watch_alert_skips (
                pk_id BIGSERIAL PRIMARY KEY,
                watch_id BIGINT NOT NULL REFERENCES crab.member_watches(pk_id) ON DELETE CASCADE,
                reason VARCHAR(50) NOT NULL,
                quoted_usd NUMERIC(10,2),
                observed_usd NUMERIC(10,2),
                source VARCHAR(50),
                deep_link TEXT,
        ...
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_watch_alert_skips_recent ON crab.watch_alert_skips(skipped_at DESC, reason)")
