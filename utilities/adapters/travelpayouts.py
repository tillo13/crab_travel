...
class TravelpayoutsAdapter(TravelAdapter):
        ...
        try:
            ...
            for fare_key, fare in dest_data.items():
                ...
                results.append(self.flight(
                    ...
                    deep_link=deep_link,
                    deep_link_honors_price=True,  # aviasales affiliate URL — TP's own surface
                    bookable=False,
                    raw=fare,
                ))
    ...
    def _get_location_id(self, destination):
        ...
        Hotellook's lookup is finicky about state suffixes ("Scottsdale AZ"
        returns 0 matches). Try the literal, then strip 2-letter state, then
        keep only what's before the first comma.
        """
        import re as _re
        candidates = [destination]
        stripped = _re.sub(r',?\s+[A-Z]{2}$', '', destination.strip())
        if stripped != destination:
            candidates.append(stripped)
        if ',' in destination:
            candidates.append(destination.split(',', 1)[0].strip())
        ...
        for q in candidates:
            ...
            try:
                resp = requests.get(
                    ...
                    params={"query": q, "lang": "en", "lookFor": "city", "token": self.token},
                    timeout=10,
                )
                resp.raise_for_status()
                cities = (resp.json().get("results") or {}).get("cities") or []
                if cities:
    ...
    def search_hotels(self, destination, checkin, checkout, guests=2):
        """
        DEPRECATED upstream: Hotellook v2 API (engine.hotellook.com/api/v2/*)
        returns 404 on every endpoint as of 2026-05. lookup.json, cache.json,
        and static/locations.json all dead. No replacement wired in yet.
        Returns [] silently rather than burning HTTP. Wire in a working
        hotel data source and restore this when available.
        """
        return []
        ...
        try:
            ...
            for h in hotels:
                ...
                results.append(self.hotel(
                    ...
                    deep_link=deep_link,
                    deep_link_honors_price=True,  # hotellook dest-search shows the same "from $X" anchor
                    bookable=False,
                    raw=h,
                ))
