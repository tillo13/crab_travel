...
class TravelAdapter:
    ...
    def flight(self, origin, destination, depart_at, arrive_at, airline,
               ...
               deep_link_honors_price=False):
        # deep_link_honors_price: True only when the URL lands on a page that
        # reliably displays this quote's price (adapter's own affiliate URL,
        # native checkout, etc). False when the URL is a synthesized fallback
        # (e.g., a generic Kayak search URL when the adapter has no real link).
        # The watch engine refuses to fire user-visible alerts on quotes whose
        # deep_link doesn't honor the price unless another adapter corroborates.
        return {
            'type': 'flight',
            ...
            'price_usd': price_usd,
            'deep_link': deep_link,
            'deep_link_honors_price': deep_link_honors_price,
            'bookable': bookable,
            'raw': raw or {},
        }
