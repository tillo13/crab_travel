...
class DuffelAdapter(TravelAdapter):
    ...
    def _normalize_offer(self, offer, origin, destination, depart_iso=None, return_iso=None):
        ...
        # Duffel test mode has no public booking URL; _build_deep_link returns
        # a Kayak search URL as a fallback. That URL doesn't honor our quoted
        # price — Kayak shows whatever Kayak shows, which is often materially
        # different from Duffel's inventory. Mark it explicitly so the watch
        # engine refuses to fire alerts on Duffel-only quotes.
        return self.flight(
            origin=origin,
            destination=destination,
            depart_at=depart_at,
