...
# ── Suppress-on-unchanged gate for status / digest emails ────────────────────
# Shared by daily_heartbeat and the openclaw admin-digest: a status email is
# re-sent only when its meaningful state changes, with a floor so an unchanged
# state still mails periodically (proves the cron + send path are alive). State
# is keyed by notification_type in crab.notifications_sent.state_hash.
def state_email_should_send(notification_type, state_hash, floor_days=7):
    """Return (should_send: bool, reason: str). Sends when there is no prior
    send for this type, OR the state_hash differs from the last send, OR at
    least floor_days have elapsed since the last send. The age comparison is
    done in SQL so the naive/UTC storage of sent_at can't skew the floor."""
    from utilities.postgres_utils import get_db_connection
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        cur.execute("""
            SELECT state_hash, (sent_at < NOW() - make_interval(days => %s)) AS floor_expired
            FROM crab.notifications_sent
            WHERE notification_type = %s
            ORDER BY sent_at DESC
            LIMIT 1
        """, (int(floor_days), notification_type))
        row = cur.fetchone()
    except Exception as e:
        # Never let the dedupe gate itself block a status email — fail open (send).
        logger.warning(f"state_email gate unavailable for {notification_type}: {e}")
        return True, 'gate-unavailable'
    finally:
        conn.close()
    if row is None:
        return True, 'first-ever'
    last_hash, floor_expired = row
    if last_hash != state_hash:
        return True, 'state changed'
    if floor_expired:
        return True, f'{floor_days}d floor'
    return False, 'unchanged'


def state_email_record(notification_type, state_hash, channel='email'):
    """Log that a status email was actually sent — the row's sent_at is the
    dead-man clock for the floor, its state_hash the comparison key for next run."""
    from utilities.postgres_utils import get_db_connection
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO crab.notifications_sent (notification_type, channel, state_hash)
            VALUES (%s, %s, %s)
        """, (notification_type, str(channel)[:32], str(state_hash)[:64]))
        conn.commit()
    except Exception as e:
        logger.warning(f"state_email record failed for {notification_type}: {e}")
        try:
            conn.rollback()
        except Exception:
            pass
