...
def record_watch_scan(watch_id, quotes):
    """Persist a multi-adapter scan in one transaction.

    quotes: list of dicts with {source, price_usd, deep_link, data}.

    Why this exists (not just calling update_watch_price per adapter): the
    alert engine computes its baseline from the full watch_history. If we
    only wrote the per-run min, history would oscillate between adapters
    and the baseline would jitter. Every adapter's quote lands in history;
    member_watches gets the min for display.

    Returns the updated watch row (RealDictCursor dict) or None.
    """
    import psycopg2.extras
    if not quotes:
        return None
    prices = [float(q['price_usd']) for q in quotes if q.get('price_usd')]
    if not prices:
        return None
    min_quote = min(quotes, key=lambda q: float(q['price_usd']))
    min_price = float(min_quote['price_usd'])
    min_deep_link = min_quote.get('deep_link')
    min_data = min_quote.get('data')
    ...
    try:
        ...
        for q in quotes:
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s)
            """, (
                watch_id, float(q['price_usd']), q.get('source', 'unknown'),
                q.get('deep_link'),
                psycopg2.extras.Json(q.get('data') or {}),
            ))
        ...
        cursor.execute("""
            ...
            RETURNING *
        """, (
            min_price, min_deep_link,
            psycopg2.extras.Json(min_data) if min_data else None,
            psycopg2.extras.Json(min_data) if min_data else None,
            min_price, min_price, min_price, watch_id,
        ))
        watch = cursor.fetchone()
        conn.commit()
        return dict(watch) if watch else None
        ...
        if conn:
            conn.rollback()
        logger.error(f"record_watch_scan failed for watch {watch_id}: {e}")
        return None
    finally:
        if cursor:
            cursor.close()
