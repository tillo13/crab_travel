...
@bp.route('/tasks/evaluate-alerts')
def task_evaluate_alerts():
    """Cron — decision tier of the watch pipeline.

    Reads recent observations from crab.watch_history (written by OpenClaw
    via /api/opencrab/watch-observations), runs the alert decision per
    watch, sends emails through the ledger-gated path. No external API
    calls — pure SQL + decision logic, finishes in seconds.

    Replaces /tasks/check-watches (which did the scanning inline on App
    Engine — moved to OpenClaw 2026-05-11).
    """
    task_secret = os.environ.get('CRAB_TASK_SECRET', 'dev')
    if not request.headers.get('X-Appengine-Cron') and request.args.get('secret') != task_secret:
        return 'Forbidden', 403
    try:
        from utilities.watch_engine import evaluate_pending_alerts
        summary = evaluate_pending_alerts()
        logger.info(f"✅ /tasks/evaluate-alerts complete: {summary}")
        return jsonify({'success': True, **summary})
    except Exception as e:
        logger.error(f"❌ evaluate-alerts failed: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def task_seed_booked_trips():
    ...
    try:
        ...
        itineraries_added = 0
        from utilities.kumori_api_client import llm_generate
        import json as _json
