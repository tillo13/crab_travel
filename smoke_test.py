...
# ── 10. Watch-alert decision pipeline ─────────────────────────

section("10. Watch alert decision pipeline (anti-flap, dedupe, deep_link contract)")
try:
    import subprocess
    proc = subprocess.run(
        [sys.executable, 'tests/test_watch_alerting.py'],
        capture_output=True, text=True, timeout=60,
    )
    if proc.returncode == 0:
        ok("tests/test_watch_alerting.py — all assertions passed")
    else:
        print(proc.stdout)
        print(proc.stderr)
        fail("watch alerting smoke tests failed")
except Exception as e:
    fail(f"watch alerting smoke runner crashed: {e}")
