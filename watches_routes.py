...
def api_watch_explore(watch_id):
    """Current prices for a single watch's route — called by the OpenCrab VPS.

    Reads the freshest per-source observations that the OpenClaw flight-hunter
    scanner already wrote to ``kumori_ops.flight_hunter_observations``. NO live
    scraping happens here.

    History: this endpoint used to fan ``ALL_ADAPTERS`` across a
    ``ThreadPoolExecutor(8)`` *on the App Engine web instance*, which OOM-killed
    the 256 MB F1 (HTTP 503, ~60×/day). Scraping now lives only on the VPS
    (discovery tier); App Engine just reads (decision tier). Same response shape
    as before, so VPS callers (daily-plan-state run.py + flight_explore adapter)
    need no change. Read-only — writes nothing to member_watches.
    """
    from flight_hunter.reader import latest_observations_for_route
    ...
    try:
        ...
        # Read observations for each (origin × dest) expansion near the checkin date,
        # dedup by canonical_key keeping cheapest. Bounded index reads — no scraping.
        by_key = {}
        routes = 0
        for origin in origin_expansions:
            for dest in dest_expansions:
                routes += 1
                for r in latest_observations_for_route(
                        conn, origin, dest, watch['checkin'],
                        date_shift_days=date_shift_days,
                        limit=max_per_combo * 12):
                    key = r.get('canonical_key') or f"{r.get('source')}:{r.get('origin')}-{r.get('destination')}"
    ...
    return jsonify({
        'success': True,
        'source': 'flight_hunter_observations',
        'watch': {
            'id': watch['pk_id'],
            'origin': watch['origin'],
